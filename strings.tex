...
constant time by extending the edge label that was previous only the
...
\begin{thm}
For a query string $s$, a Patricia tree can report all the strings it contains that have $s$ as a prefix and this can be done in $O(|s|+k)$ time, where $k$ is the number of strings output by the query.
\end{thm}
